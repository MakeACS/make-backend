...
const RUNTIME_CLIENT_RUNTIME_PATH = "/@vite-plugin-eslint2-runtime";
const RUNTIME_CLIENT_ENTRY_PATH = "/@vite-plugin-eslint2-runtime-entry";
const PLUGIN_OPTION_KEYS = [
	"test",
	"dev",
	"build",
	"include",
...
const getESLintConstructorOptions = (options) => ({
	...Object.fromEntries(Object.entries(options).filter(([key]) => !PLUGIN_OPTION_KEYS.includes(key))),
	errorOnUnmatchedPattern: false
});
const initializeESLint = async (options) => {
	const { eslintPath, formatter } = options;
	try {
		const module = interopDefault(await import(eslintPath));
		...
		return {
			eslintInstance,
			formatter: await eslintInstance.loadFormatter(formatter),
...
const filterResults = (results, options) => {
	...
	if (!options.emitError) filtered = removeESLintErrorResults(filtered);
	if (!options.emitWarning) filtered = removeESLintWarningResults(filtered);
	filtered = filterESLintLintResults(filtered);
	let textType;
	if (filtered.some((result) => result.errorCount > 0)) textType = options.emitErrorAsWarning ? "warning" : "error";
	else textType = options.emitWarningAsError ? "error" : "warning";
	return {
		results: filtered,
		textType
	};
};
const report = async (results, formatter, textType, context, emit) => {
	...
	else log(formattedText, textType, context);
};
/**
...
*/
const buildOverlayPayload = (results, textType) => ({ results: results.map((result) => ({
...
var OverlayManager = class {
	...
	upsert(payload) {
		for (const result of payload.results) if (result.messages.length === 0) this.files.delete(result.filePath);
		else this.files.set(result.filePath, result);
...
function createLinter(options, adapters) {
	...
	const overlayManager = adapters?.onOverlayPayload ? new OverlayManager() : void 0;
	const lintFiles = async (files, context) => {
		await ready;
		...
		if (results.length === 0) {
			if (overlayManager && adapters?.onOverlayPayload) {
				overlayManager.upsert({ results: lintResults.map((result) => ({
					filePath: result.filePath,
					messages: []
				})) });
		...
		if (overlayManager && adapters?.onOverlayPayload) {
			const payload = overlayManager.upsert(buildOverlayPayload(results, textType));
	...
	return {
		lint: async (id, context) => {
			await ready;
			if (await shouldIgnoreModule(id, filter, eslintInstance)) return;
			const filePath = getFilePath(id);
			return lintFiles(options.lintDirtyOnly ? filePath : options.include, context);
		},
...
Object.defineProperty(exports, "RUNTIME_CLIENT_ENTRY_PATH", {
	enumerable: true,
	get: function() {
		return RUNTIME_CLIENT_ENTRY_PATH;
	}
});
Object.defineProperty(exports, "RUNTIME_CLIENT_RUNTIME_PATH", {
	enumerable: true,
	get: function() {
		return RUNTIME_CLIENT_RUNTIME_PATH;
	}
});
Object.defineProperty(exports, "WS_OVERLAY_EVENT", {
	enumerable: true,
	get: function() {
		return WS_OVERLAY_EVENT;
	}
});
Object.defineProperty(exports, "WS_RUNTIME_LOADED_EVENT", {
	enumerable: true,
	get: function() {
		return WS_RUNTIME_LOADED_EVENT;
	}
});
...
Object.defineProperty(exports, "createLinter", {
	enumerable: true,
	get: function() {
		return createLinter;
	}
